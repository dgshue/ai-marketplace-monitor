...
| `max_images`          | Optional          | Integer             | How many of a listing's photos to keep once it reaches the review threshold. Default: 6, maximum 12, `0` keeps none. Facebook only.                          |
| `rating`              | Optional          | Integer/List        | Notify users with listings with rating at or higher than specified rating.                                                                                  |
| `review_rating`       | Optional          | Integer/List        | Show listings with rating at or higher than this in the web UI's review queue. Must be at or below `rating`. Default: 3.                                     |
...
3. `prompt`, `extra_prompt`, `rating_prompt`, `review_rating`, and `rating` are used to adjust how to interact with an AI service. See [Adjust prompt and notification level](../README.md#adjust-prompt-and-notification-level) for details.
...
5. A list of two values can be specified for options `rating`, `review_rating`, `availability`, `delivery_method`, and `date_listed`. See [First and subsequent searches](../README.md#first-and-subsequent-searches) for details.
